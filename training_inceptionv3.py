...
from utils import k_fold_splitter, create_training_data, aug_rot, aug_crop, aug_cutout, aug_shear, aug_gblur, aug_randcomb, train_model_inceptionv3, read_args, aug_mobius, shift_func, mobius_fast_interpolation, getabcd_1fix, M_admissable, madmissable_abcd, create_training_data
...
if mobius:
  ...
  for i in range(0, k):
    split_data.append(tdata[int(round(len(tdata)*((i)/k), 0)):int(round(len(tdata)*((i+1)/k), 0))])
    split_aug_data.append(aug_mobius(split_data[i], M=2, mode='wrap', user_defined=False, rgb=True))   # M must be > 1, and this is slower the closer to that
...
for i in range(0, len(training_list)):
    ...
    valaccs.append(train_model_inceptionv3(training_list[i], val_list[i], savename + '{}'.format(i)))
