...
if load:
  ...
  print("ratios of labels in the test set are {} : {} : {}".format(round(y_test.count(0) / len(y_test), 2),
    ...
    data = create_data(path, duplicate_channels=False, equalize=False)
...
plot_counts(label_count, haralick=True)

plot_scatter(k_means_labels, kmeans, PCA_components, haralick=True)
